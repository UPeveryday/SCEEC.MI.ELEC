...

{
    ...
    public class PortClass
    {
        ...
        public PortClass(string comPortName)
        ...
        public PortClass(string comPortName, int baudRate, Parity parity, int dataBits, StopBits stopBits)
        ...
        public byte[] ReadPortsData(byte[] SendData, byte[] RecData, int RecDataLength, int Timeout)
        {
            if (_serialPort.IsOpen)
            {
                ...
                if (count >= RecDataLength && count != 0)
                {
                    while (offset < count)
                    {
                        SingalReadDataLength = _serialPort.Read(RecData, offset, count - offset);
